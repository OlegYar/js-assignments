...
function isInsideCircle(circle, point) {
    let length = Math.sqrt(Math.pow((point.x - circle.center.x), 2) + Math.pow((point.y - circle.center.y), 2));
...
function findFirstSingleChar(str) {
    return str.split('').find((item, index, arr) => arr.indexOf(item) == arr.lastIndexOf(item));
}
...
function evaluateTicTacToePosition(position) {
    ...
    if (position[2][0] != undefined && position[2][0] == position[1][1] && position[2][0] == position[0][2]) {
        ans = position[2][0];
        return ans;
    }
    return ans;
